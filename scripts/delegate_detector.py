...
class DelegateDetector:
    ...
    def _predict_threshold(self, config, model, item):
        ...
        # Apply dynamic threshold if supported by detector
        if hasattr(self.detector, 'get_dynamic_threshold'):
            threshold = self.detector.get_dynamic_threshold(threshold)
    ...
    def _eval(self, results, config, model, label_fn):
        ...
        for item in results:
            ...
            # For dynamic thresholding, we need to recompute detection criteria
            # to get the perplexity for each text
            if hasattr(self.detector, 'get_dynamic_threshold'):
                field = self.feature_fields[0]
                if field in item:
                    target = item[field]
                    # Recompute to get perplexity
                    crit = self.detector.compute_crit(target)
                    item[f'{field}_crit'] = crit
